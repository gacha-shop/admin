...
export const SHOP_TYPE_OPTIONS = [
  { value: "gacha" as const, label: "가챠" },
  { value: "figure" as const, label: "피규어" },
  { value: "claw" as const, label: "클로우" },
] as const;
 ...
 */
export type DaySchedule = { closed: true } | { open: string; close: string };
...
export interface CreateStoreFormData {
  // Basic information
  name: string;
  shop_type: ShopType; // Array of shop types
  description: string;
  phone: string;
  social_urls: SocialUrls | null;
  tag_ids: string[];
