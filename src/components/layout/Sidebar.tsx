...
export function Sidebar() {
  ...
  // Convert MenuWithChildren to MenuItem format
  const convertMenusToMenuItems = (menus: MenuWithChildren[]): MenuItem[] => {
    return menus.map((menu) => {
      // Get icon component from Lucide
      const IconComponent = menu.icon
        ? (LucideIcons[menu.icon as keyof typeof LucideIcons] as LucideIcon)
        : LayoutDashboard;

      const menuItem: MenuItem = {
        label: menu.name,
        icon: IconComponent || LayoutDashboard,
        requiresSuperAdmin: menu.metadata?.requiresSuperAdmin || false,
      };

      // If has path and no children, it's a direct link
